...
In the remainder of this tutorial we will edit the file together, 
 gradually constructing the pipeline which reproduces the results from MRW.
...
The indendation here is important.
...
Snakemake uses the indendation to figure out different components of the each rule.
...
The fact that Snakemake is written in Python is why indendation matters.
...
In the next chapter, we will then add `gen_reg_vars.R`, which creates additional variables to be used in subsequant steps.
...
2. a `--out` option which defines the filepath the transformed data set will be saved.
   For us, we will save the output to the file `out/data/mrw_renamed.csv`
...
We can think of the `input` as ingredients, the `output` as what comes out of the recipe and the `shell` part as the recipe itself.
...
Let's now update the `rename_vars` rule we created above using the input and output syntax from above.
...
Looking at our rule so far, there is a lot of repetition, we write in the names of all the files first in `input` and `output` before writing them again inside the `shell` command.
Snakemake does not require us to write these paths twice.
We can refer to the inputs and outputs in the shell part of our rule via the names we assigned them.
...
To understand the status of the project as seen by Snakemake, we can use the **summary** of Snakemake:
...
Snakemake keeps track of dependencies and updates to input files and only re-runs a rule if an input has changed.
This means we the researcher no longer have to think about when we last ran a script and if we need to run it again.
Snakemake's management of dependencies will turn out to be quite advantageous as you develop a larger projects.
No longer does one need to think about order of execution and when to re-run files.^[
...
In our motivation chapter we explained how our final workflow should be a connection between a set of rules,
 which we depicted grahically as:

```{r, fig.align="center", echo=FALSE}
knitr::include_graphics("img/01-motivation/rulegraph.png")
```

So far, we have constructed the first of those blocks.
That is, our set of rules in our current workflow looks like this:

```{r, fig.align="center", echo=FALSE}
knitr::include_graphics("img/11-initial-steps/rulegraph.png")
```

So we have made the first step in the data analysis pipeline.
