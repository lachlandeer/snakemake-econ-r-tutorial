...
Currently, our project has three rules which perform OLS regressions parallel to one another at the top of the file `Snakefile`.
As only one rule can be the target rule, it would require us to the execute Snakemake three times -- once for every OLS regression:
...
Now that we understand the concept of `target rules`, we will try to make use of them for our advantage.

As we know, Snakemake will execute any rule which can produce an output which a target rule requires as its input.
Adding a new dedicated target rule which requires the outputs of our three solow models and does nothing else otherwise, will allow us to execute all three rules via a single line of code.
